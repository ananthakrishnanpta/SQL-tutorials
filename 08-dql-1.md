...
| `CHAR`             | `CAST(CHAR(n) AS CHAR)`                                                              | Converts an ASCII code to a character. The CAST function ensures that the returned value is casted as character instead of binary or hex value.                 | `SELECT CHAR(65);` (returns 'A')         |
